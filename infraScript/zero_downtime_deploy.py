...
class ServiceManager:
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --network=chibbotec-network --network-alias=auth "
            f"--restart unless-stopped -p {port}:9010 -e TZ=Asia/Seoul "
            f"-v /dockerProjects/chibbotec/auth_service/volumes/gen:/gen "
            f"--pull always ghcr.io/chibbotec/auth_service")
    ##
